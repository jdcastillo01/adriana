...
MSG_VENTAS = "Buen dia {representante}!, quisiera validar contigo para montar el pedido del mes."
...
def init_db():
    with sqlite3.connect(DB_NAME) as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS clientes (
            ...
            nombre TEXT NOT NULL, representante TEXT, telefono TEXT NOT NULL, estatus TEXT DEFAULT 'Por vender')''')
        try:
            conn.execute('ALTER TABLE clientes ADD COLUMN representante TEXT')
...
def ir_whatsapp(tabla, id):
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if c:
            if tabla == 'ventas':
                rep = c['representante'] if 'representante' in c.keys() and c['representante'] else ''
                msg = MSG_VENTAS.format(nombre=c['nombre'], representante=rep)
            else:
                msg = MSG_COBRANZA.format(nombre=c['nombre'], factura=c['factura'], monto=c['monto'])
        ...
        try:
            ...
            with sqlite3.connect(DB_NAME) as conn:
                for _, row in df.iterrows():
                    ...
                    rep = row.get('nombre representante') or row.get('representante') or row.get('contacto')
                    representante = "" if pd.isna(rep) else str(rep).strip()
                    if representante.lower() == "nan": representante = ""
                    tel = re.sub(r'[^\d+]', '', str(row.get('telefono') or ""))
                    if nombre and not pd.isna(nombre):
                        if tipo == 'ventas': 
                            conn.execute('INSERT INTO clientes (nombre, representante, telefono) VALUES (?, ?, ?)', (str(nombre), representante, tel))
                        else:
                            f = row.get('nro. factura') or "S/N"
                            m = row.get('total factura') or "0.00"
                            # NUEVA LÓGICA: Captura el estatus del Excel si existe
                            e_excel = str(row.get('estatus') or "").strip()
                            estatus_final = "En Mora" if e_excel.lower() == "en mora" else "En espera de pago"
                            
                            conn.execute('INSERT INTO cobranza (nombre, telefono, factura, monto, estatus) VALUES (?, ?, ?, ?, ?)', 
                                         (str(nombre), tel, str(f), str(m), estatus_final))
